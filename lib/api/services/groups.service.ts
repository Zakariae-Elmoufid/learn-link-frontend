...
export const groupsService = {
  ...
  create: (data: CreateGroupRequest) =>
    apiClient.post<StudyGroupResponse>("/groups", data).then((r) => r.data),

  /** Get group details */
  getById: (groupId: number) =>
    apiClient.get<StudyGroupResponse>(`/groups/${groupId}`).then((r) => r.data),
  ...
  delete: (groupId: number) => apiClient.delete(`/groups/${groupId}`),
  ...
  discover: (page = 0, size = 10) =>
    apiClient
      ...
      >("/groups/discover", { params: { page, size } })
      .then((r) => r.data),
  ...
  search: (keyword: string, page = 0, size = 10) =>
    apiClient
      .get<
        PageResponse<StudyGroupResponse>
      >("/groups/search", { params: { keyword, page, size } })
      .then((r) => r.data),
  ...
  getMyGroups: () =>
    apiClient.get<StudyGroupResponse[]>("/groups/my").then((r) => r.data),

  /** Get groups owned by current user */
  getOwnedGroups: () =>
    apiClient.get<StudyGroupResponse[]>("/groups/owned").then((r) => r.data),

  // ─── Membership ──────────────────────────────────────────────────────────

  /** Join a public group */
  join: (groupId: number) => apiClient.post(`/groups/${groupId}/join`),
  ...
  /** Leave a group */
  leave: (groupId: number) => apiClient.post(`/groups/${groupId}/leave`),
  ...
  updateMemberRole: (groupId: number, memberId: number, role: GroupRole) =>
    apiClient.put(`/groups/${groupId}/members/${memberId}/role`, null, {
      params: { role },
    }),
